...
Living Plot is a mobile interactive-drama app where each 60–90 second scene ends with three choices and the next scene reflects the user's canonical committed branch.
...
The Worker also binds the `living_plot_events` Analytics Engine dataset. Scene-generation telemetry contains provider/model/outcome, provider-reported token counts, pricing revision, and integer nano-USD cost only. Product funnel telemetry records only newly canonical bounded events such as drama creation, committed choice, next scene, archive/restore, and fresh voice request. Neither telemetry family contains drama text or user identifiers, and neither is an authorization or quota source.
...
The canonical runtime ownership map is [`docs/arch/drama-runtime.md`](docs/arch/drama-runtime.md). Phase 1 scope and architecture are indexed in [`docs/index.md`](docs/index.md).
