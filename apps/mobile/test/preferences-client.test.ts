...
describe('preferences clients', () => {
  it('loads and saves validated live preferences with authenticated transport', async () => {
    const fetcher = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => Response.json({
      preferences: {
        ...
        storyLocale: init?.method === 'POST' ? 'vi-VN' : 'en-US',
    ...
    await expect(client.load()).resolves.toMatchObject({ uiLocale: 'en', storyLocale: 'en-US' });
    await expect(client.save({ uiLocale: 'vi', storyLocale: 'vi-VN', narratorVariant: 'vi-narrator-female' })).resolves.toMatchObject({ uiLocale: 'vi', updatedAt: 123 });
  ...
  it('rejects unsupported server values and keeps preview preferences local', async () => {
    ...
    const preview = new PreviewPreferencesClient({ uiLocale: 'en', storyLocale: 'en-US', narratorVariant: 'en-narrator-female', updatedAt: null });
    await preview.save({ uiLocale: 'vi', storyLocale: 'vi-VN', narratorVariant: 'vi-narrator-female' });
    await expect(preview.load()).resolves.toMatchObject({ uiLocale: 'vi', storyLocale: 'vi-VN' });
  });
});
