...
describe('GeminiStoryGenerator', () => {
  it('sends a structured-output request without leaking the API key into the body', async () => {
    ...
    const generator = new GeminiStoryGenerator('test-api-key', fetcher);
  ...
  it('retries exactly once for an invalid structured proposal and sums usage', async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      value: { attempts: 2, usage: { inputTokens: 110, outputTokens: 50 }, provider: 'gemini', model: STORY_MODEL },
    });
  ...
  it('does not retry provider HTTP failures', async () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: { code: 'provider_unavailable', message: 'Story provider rejected the request.', retryable: true },
    });
  });
