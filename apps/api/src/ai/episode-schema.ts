...
export function parseAndValidateEpisodeProposal(
  raw: string,
  input: EpisodeGenerationInput,
): Result<EpisodeProposal, string[]> {
  let value: unknown;
  try {
...
function parseProposal(value: unknown): Result<EpisodeProposal, string[]> {
  ...
  const choices: EpisodeChoiceProposal[] = [];
...
function parseChoice(value: unknown): Result<EpisodeChoiceProposal, string[]> {
...
function parseStateDelta(value: unknown): Result<EpisodeChoiceProposal['stateDelta'], string[]> {
...
function validateBusinessRules(proposal: EpisodeProposal, input: EpisodeGenerationInput): string[] {
  const errors: string[] = [];
  ...
  if (wordCount < 100 || wordCount > 300) errors.push('Episode script must stay within the Phase 1 spoken-length envelope.');
