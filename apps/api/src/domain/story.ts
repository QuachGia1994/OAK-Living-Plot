export type PlotStatus = 'active' | 'completed' | 'archived';
export type EpisodeStatus = 'ready' | 'completed';
...
export interface StructuredPlotState {
  schemaVersion: 2;
...
export interface EpisodeChoiceContract {
  id: string;
...
export interface PlotMemorySnapshot {
  id: string;
  userId: string;
  status: PlotStatus;
  summary: string;
...
export function createInitialPlotState(): StructuredPlotState {
  return {
    schemaVersion: 2,
    relationships: [],
    facts: [],
...
export function requireThreeChoices<T extends Pick<EpisodeChoiceContract, 'position' | 'label'>>(
  choices: readonly T[],
): readonly [T, T, T] {
  if (choices.length !== 3) {
    throw new DomainInvariantError('An episode must have exactly three choices.');
  }
  ...
  if (positions.size !== 3 || !positions.has(1) || !positions.has(2) || !positions.has(3)) {
    throw new DomainInvariantError('Episode choices must use positions 1, 2, and 3 exactly once.');
  }
  if (!labelsAreValid) {
    throw new DomainInvariantError('Episode choice labels cannot be blank.');
  }

  return [choices[0], choices[1], choices[2]];
}

export function parseStructuredPlotState(raw: string): StructuredPlotState {
  const value: unknown = JSON.parse(raw);
  if (!isRecord(value)) throw new DomainInvariantError('Invalid plot state.');

  if (value.schemaVersion === 2) return parseStateV2(value);
  return upgradeLegacyState(value);
}

function parseStateV2(value: Record<string, unknown>): StructuredPlotState {
  if (!Array.isArray(value.relationships) || !Array.isArray(value.facts) || !Array.isArray(value.openThreads)) {
    throw new DomainInvariantError('Invalid plot state collections.');
  }
  ...
  const tone = requireText(value.tone, 'Invalid plot tone state.');
...
function upgradeLegacyState(value: Record<string, unknown>): StructuredPlotState {
